...
namespace Planner
{
    ...
    bool RPGBuilder::allow_all = true;
...
//vector<RPGBuilder::Constraint> RPGBuilder::constraints;
    ...
    void RPGBuilder::RPtoDOT(std::vector<string> instances){
        cout << "RPtoDOT start" << endl;

        RPGHeuristic* currRPG = RPGBuilder::getHeuristic();
        ...
        for (int i = 0; i < instances.size(); i++) {
            for (LiteralSet::iterator lit = initialState.begin(); lit != initialState.end();){
                Literal* current_fact = *lit;
                std::stringstream ss;
                current_fact->output_to_stringstream(ss);
                if ((ss.str().find(" "+instances[i]+" ") != std::string::npos) || (ss.str().find(" "+instances[i]+")") != std::string::npos)){
                    initialState.erase(lit++);
                    // cout << "erased: " << ss.str() << "\n";
                    erased++;
                }
                else{
        ...
        if (instances.size() == 0){
            // get top level goals
            cout << "Searching for Relaxed Plan...";
            vector<string> top_goals;
            ...
            for (VAL::pc_list<VAL::goal*>::const_iterator eit = conj_goals->getGoals()->begin(); eit != conj_goals->getGoals()->end(); eit++) {
                ...
                ss << ")";
                top_goals.push_back(ss.str());
            }

            bool goal_state = true;
            for (int i = 0; i < top_goals.size(); i++){
                bool goal_found = false;
                std::ifstream rpgDot; rpgDot.open("rpg0.dot");
                std::string line;
                //cout << top_goals[i] << endl;
                while (getline(rpgDot, line)) {
                    //cout << line << endl;
                    if(line.find(top_goals[i], 0) != string::npos){
                        goal_found = true;
                        break;
                    }
                }
                if (!goal_found) goal_state = false;
            }
            if (goal_state){
                cout << endl << "Relaxed Plan found " << goal_state << endl << endl;
            }
            else cout << endl << "Relaxed Plan NOT found " << goal_state << endl << endl;
        }
        cout << "RPtoDOT end" << endl;
    }

    int RPGBuilder::getPriorityOfType(string type, std::vector<agent> agents){
        for(int i = 0; i < agents.size(); i++){
            if(agents[i].type.compare(type) == 0) return agents[i].priority;
        }
        return -1;
    }

    int RPGBuilder::isParentType(string type, std::vector<agent> agents){
        int to_return = -1;
        for(int i = 0; i < agents.size(); i++){
            if(agents[i].type.compare(type) == 0) {
                if (agents[i].is_parent){ to_return = 0;}
                else { to_return = 1;}
            }
        }
        return to_return;
    ...
    bool RPGBuilder::hasConnectedMap(std::map <string,std::vector<string>> objectsMap, std::vector<agent> agents){
        ...
        cout << "Creating Single Agents Problem:" << endl;

        //creating single agents initial state
        cout << "Filtering Initial State:" << endl;
        for (map <string,std::vector<string>>::iterator oit = objectsMap.begin(); oit != objectsMap.end(); oit++){
            ...
            for (int i = 1; i < oit->second.size(); i++) {
                for (LiteralSet::iterator lit = initialState.begin(); lit != initialState.end();) {
                    ...
                    std::stringstream ss;
                    current_fact->output_to_stringstream(ss);
                    if ((ss.str().find((" " + oit->second[i] + " "), 0) != std::string::npos) ||
                        (ss.str().find((" " + oit->second[i] + ")"), 0) != std::string::npos)) {
                        initialState.erase(lit++);
                        cout << "   erased from initial state: " << ss.str() << "\n";
                    } else lit++;
                }
            }
        }

        //creating single agents goal sate
        cout << "Filtering Goal State:" << endl;
        for (map <string,std::vector<string>>::iterator oit = objectsMap.begin(); oit != objectsMap.end(); oit++){
            if(RPGBuilder::isParentType(oit->first,agents) != 0) continue;
            for (int i = 1; i < oit->second.size(); i++) {
                for (vector<string>::iterator git = top_goals.begin(); git != top_goals.end();){
                    string goal = *git;
                    if ((goal.find((" " + oit->second[i] + " "), 0) != std::string::npos) ||
                        (goal.find((" " + oit->second[i] + ")"), 0) != std::string::npos)) {
                        cout << "   erased from goal state: " << goal << "\n";
                        top_goals.erase(git++);
                        break;
                    } else git++;
                }
        ...
        std::sort(top_goals.begin(), top_goals.end());
        top_goals.erase(std::unique(top_goals.begin(), top_goals.end()), top_goals.end());
        ...
        // get top level goals
        cout << "Searching for Relaxed Plan..." << endl;
        bool goal_state = true;
        for (int i = 0; i < top_goals.size(); i++){
            bool goal_found = false;
            std::ifstream rpgDot; rpgDot.open("rpg0.dot");
            ...
            while (getline(rpgDot, line)) {
                //cout << line << endl;
                if(line.find(top_goals[i], 0) != string::npos){
                    cout << "goal " << top_goals[i] << "found in:\n" << line << endl;
                    goal_found = true;
                    break;
                }
            }
            if (!goal_found){
                goal_state = false;
                cout << "   goal NOT found: " << top_goals[i] << endl;
            }
        }
        if (goal_state){
            cout << endl << "Relaxed Plan found " << goal_state << endl << endl;
        ...
        cout << endl << "Relaxed Plan NOT found " << goal_state << endl << endl;
        return false;
...
//         literalGoalSoftDeadlines.resize(literalGoals.size());
        ...
        findSelfMutexes();
        cout << "doSomeUsefulMetricRPGInference" << endl;
        ...
        if (doTemporalAnalysis) {
            ...
            set<int> f;
            cout << "debug 17" << endl;
            globalHeuristic = generateRPGHeuristic(f);
            cout << "debug 18" << endl;
        }
        cout << "debug 19" << endl;
