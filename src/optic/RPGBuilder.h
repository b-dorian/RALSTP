...
public:
    ...
    static map<double, ClusterDef > factLayerNodes;
    static map<double, ClusterDef > actionLayerNodes;
    static list<string> endActionsToStarts;
    static list<string> factsToActions;
    static map<int, string> achieverForFact;
...
//     }
    ...
    static void RPtoDOT(std::vector<string> instances);
    static int getPriorityOfType(std::string type, std::vector<agent> agents);
    static int isParentType(std::string type, std::vector<agent> agents);
    static bool hasConnectedMap(std::map <string,std::vector<string>> objectsMap, std::vector<agent> agents);
    ...
    static RPGHeuristic * generateRPGHeuristic(const set<int> & forbidden_actions);
    static RPGHeuristic * getHeuristic() {
        if (!globalHeuristic) {
        	set<int> f;
            globalHeuristic = RPGBuilder::generateRPGHeuristic(f);
        }
...
public:
    ...
    static vector<EpsilonResolutionTimestamp> & getEarliestPropositionPOTimes();
    ...
    const map<Literal*, set<pair<int, Planner::time_spec> > > &get_fact_achievers() const;
};
