...
//    };
    map<double, RPGBuilder::ClusterDef > factLayerNodes;
    map<double, RPGBuilder::ClusterDef > actionLayerNodes;
    ...
    bool rawHighlight(const string & s, const bool failureOkay=false) {
        bool found = false;
        map<double, RPGBuilder::ClusterDef >::iterator alItr = actionLayerNodes.begin();
        const map<double, RPGBuilder::ClusterDef >::iterator alEnd = actionLayerNodes.end();
    ...
    void printLayerDef(ostream & o, const bool & factLayer, const double & atTime, const int & idx, const RPGBuilder::ClusterDef & def) const {
...
ostream & operator <<(ostream & o, const DotDetails & d) {
    ...
    map<double, RPGBuilder::ClusterDef >::const_iterator flItr = d.factLayerNodes.begin();
    const map<double, RPGBuilder::ClusterDef >::const_iterator flEnd = d.factLayerNodes.end();
    
    map<double, RPGBuilder::ClusterDef >::const_iterator alItr = d.actionLayerNodes.begin();
    const map<double, RPGBuilder::ClusterDef >::const_iterator alEnd = d.actionLayerNodes.end();
...
public:
	set<int> forbidden_actions;
	map<Literal*, set<pair<int, Planner::time_spec> > > fact_achievers;
...
vector<EpsilonResolutionTimestamp> & RPGHeuristic::getEarliestPropositionPOTimes()
{
    return Private::earliestPropositionPOTimes;
};
...
const map<Literal*, set<pair<int, Planner::time_spec> > > &RPGHeuristic::get_fact_achievers() const
{
	return d->fact_achievers;
}
...
RPGHeuristic::RPGHeuristic(const bool & b,
                           ...
                           list<pair<int, Planner::time_spec> > * pla,
                           list<pair<int, Planner::time_spec> > * onpa,
                           const set<int> &forbidden_actions)
        :   d(new Private(b,
                          atse,
                          atee,
                          eta,
...
RPGHeuristic* RPGBuilder::generateRPGHeuristic(const set<int> &forbidden_actions)
{
...
//             }
    ...
    if ((payload->unsatisfiedGoals > payload->fakeGoalCount) || payload->unappearedEnds
        ...
        if (RPGHeuristic::printRPGAsDot) {
            ...
            RPGBuilder::factLayerNodes = dot.factLayerNodes;
            RPGBuilder::actionLayerNodes = dot.actionLayerNodes;
            RPGBuilder::endActionsToStarts = dot.endActionsToStarts;
            RPGBuilder::factsToActions = dot.factsToActions;
            RPGBuilder::achieverForFact = dot.achieverForFact;
                                                      ...
                                                      const bool & openEnd, const EpsilonResolutionTimestamp & nlTime)//, MaxDependentInfo & POtime)
{
...

    {
        ...
        for (; addEffItr != addEffEnd; ++addEffItr) {
            ...
            fact_achievers[*addEffItr].insert(make_pair(currAct, currTS));
                                                           ...
                                                           const bool & decrementRemainingUnsatisfiedPreconditionCounters)
{
    ...
    for (; depItr != depEnd; ++depItr) {
        ...
        int currAct_ID = currAct;
        if (forbidden_actions.find(currAct_ID) != forbidden_actions.end())
        	continue;
        ...
        if (startAct) {
            ...
            if ((!spc && !numericStartPreconditionCounts[currAct]) && forbiddenStart.find(currAct) == forbiddenStart.end() && nlTime < latestStartAllowed[currAct]) {
                ...
                if (expandFully) {
                	...
                	//RPGBuilder::getInstantiatedOp(currAct)->write(cout);
                	//cout << "(" << currAct << ") : " <<  factLayerTime << endl;
                }
...
bool RPGHeuristic::printRPGAsDot = true;//false;
