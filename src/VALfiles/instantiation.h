...
public:
    ...
    bool abstract = false;
    VAL::proposition * abstract_prop = NULL;
	...
	void write(ostream & o) const
	{
        VAL::proposition * target_prop = const_cast<VAL::proposition *>(prop);
        if (abstract){
            target_prop = abstract_prop;
        }
		o << "(" << target_prop->head->getName();
				transform(target_prop->args->begin(),target_prop->args->end(),
    ...
    void output_to_stringstream(std::stringstream & ss) const
    {
        ...
        ss << "(" << target_prop->head->getName();
        transform(target_prop->args->begin(),target_prop->args->end(),
                  FlexiblePrint<string>(ss," ",""),LiteralParametersOutput(*env));
        ss << ")";
    };
...
public:
    ...
    void output_to_stringstream(std::stringstream & ss) const {
        ss << "(" << op->name->getName();
        std::transform(op->parameters->begin(),op->parameters->end(),
                           FlexiblePrint<string>(ss," ",""),ActionParametersOutput(*env));
        ss << ")";
    }

    const VAL::operator_ * getOp() const
    {
        return op;
    };

    const VAL::FastEnvironment * getEnv() const
    {
        return env;
    };
